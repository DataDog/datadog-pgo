...
func (c *Client) SearchProfiles(ctx context.Context, query SearchQuery) (profiles []*SearchProfile, err error) {
	...
	if err := c.post(ctx, "/api/unstable/profiles/list", query, &response); err != nil {
		return nil, err
	}
...
// the response.
func (c *Client) post(ctx context.Context, path string, payload any, response any) error {
	...
	if res.StatusCode < 200 || res.StatusCode >= 300 {
		return fmt.Errorf("POST %s: %s: please check that your DD_API_KEY, DD_APP_KEY and DD_SITE env vars are set correctly", path, res.Status)
	}
